  ...
  try {
    const roleResult = await pool.request()
      ...
      .query(`
        ...
        -- Aggregate Required Skills with MinYears formatted
        OUTER APPLY (
          SELECT STRING_AGG(
          ...
          WHERE rs.RoleID = ro.RoleID AND rs.IsRequired = 1
        ) skReq

        -- Aggregate Nice-To-Have Skills
        OUTER APPLY (
          SELECT STRING_AGG(
            CONCAT(sl.SkillName, CASE WHEN rs.MinYears > 0 THEN CONCAT(' (', CAST(rs.MinYears AS VARCHAR(10)), ' yrs)') ELSE '' END),
            ', '
          ...
          FROM dbo.RoleSkills rs
          JOIN dbo.SkillLibrary sl ON rs.SkillID = sl.SkillID
          WHERE rs.RoleID = ro.RoleID AND (rs.IsRequired = 0 OR rs.IsRequired IS NULL)
        ) skNice

        -- Aggregate Required Certifications
        OUTER APPLY (
          SELECT STRING_AGG(rc.CertificationName, ', ') AS RequiredCertifications
          FROM dbo.RoleCertifications rc
          WHERE rc.RoleID = ro.RoleID
        ) cert
